...
8. **Clarification round:** if any per-repo or `C_CR*` clarify item is non-empty, stop and ask via skill **`hitl-choice`** preset **Engineer-review clarify** (sequential `AskQuestion` per `C#` / `C_CR#`; recommended option labeled; tokens `api:C1:A`, `C_CR1:B`; batch text OK). Never ask a clarify item without repeating that item’s File, Lines, Jump, and numbered code fence in the question — Jump path is not enough. Route answers:
