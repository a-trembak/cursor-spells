...
- **Pipeline continue after miss (spine step 15):** after Teach-review miss is handled on a pipeline run (`/csp-start-task` / `finish-plan` / `/csp-start-task --fast` / `/csp-start-issue-task`) — whether `no_miss`, `teach-review` returned, or `project_secret` capture settled — invoke skill `propose-commit` once, then full-path `update-docs` / residual propose / **`local-verify`** / `create-pr` (or fast/issue **`local-verify`** / `create-pr`) as on the skill spine. Do not end the turn on the teach land report. Manual `/csp-engineer-review` does **not** auto-start `propose-commit` or `local-verify` unless the human asks.
