...
2. If this invocation follows a **finished plan** and the user has not yet said `skip` / `approve` / `done`, stop and ask via skill `hitl-choice` (AskQuestion required; or tell them to run `/finish-plan`). Do not dispatch phases. Do **not** load the merge feedback pack yet.
3. Manual `/engineer-review` → proceed immediately per the skill spine.
...
Dispatch per skill spine: `review-lint`, `review-logic`, `review-patterns`, `review-deadcode`, `review-simplify`, `review-architecture`, `review-performance`, `review-security` (conditional), `review-figma-markup`, `review-learn` (`mode:load` before phases; `mode:capture` only after `project_secret`).
...
- Never load full third-party skill text, ledger markdown, `learned-misses.md`, or interaction-replay / auth-rtk / figma-markup / responsive-layout / null-safety / jpa-criteria / styling checklist bodies into this orchestrator context — phases and `review-learn` own those reads.
...
- Never let a heuristic phase hand-edit code to satisfy a lint rule — mechanical style/lint findings belong to `review-lint`.
...
- **Teach-review miss:** after the validated report, ask via `hitl-choice` preset **Teach-review miss** (`miss` / `project_secret` / `no_miss`). Do not auto-capture. `project_secret` → dispatch `review-learn` `mode:capture` only.
- **Pipeline continue after miss (spine step 15):** after Teach-review miss is handled on a pipeline run (`/start-task` / `finish-plan` / `/start-task --fast` / `/start-issue-task`) — whether `no_miss`, `teach-review` returned, or `project_secret` capture settled — invoke skill `propose-commit` once, then full-path `update-docs` / residual propose / `create-pr` (or fast/issue `create-pr`) as on the skill spine. Do not end the turn on the teach land report. Manual `/engineer-review` does **not** auto-start `propose-commit` unless the human asks.
