...
12. **Merge → evidence gate → feedback (mandatory):**
   ...
   - Coverage notes `graphify: used|absent|unqueryable`. When auth/session **or** interactive overlay/filter was in scope: **must** include `interaction_replay: auth|overlay-focus|both|skipped|n/a` (**R7**); optional `auth_flow_walk: …`.
