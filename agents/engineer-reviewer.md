...
13. **Merge → evidence gate → feedback** per `feedback-format.md` + `evidence-gate.md`. Validate with `validate-review-report.sh`. Coverage: `graphify:…`, `review_learnings:…`, and when in scope `interaction_replay:…` (**R7**) and `narrow_viewport:…` (**V4**).
...
- Never load full third-party skill text, ledger markdown, or R1–R7 / V1–V4 checklist bodies into this orchestrator context — phases and `review-learn` own those reads.
