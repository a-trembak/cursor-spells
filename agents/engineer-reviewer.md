...
13. **Merge → evidence gate → feedback** per `feedback-format.md` + `evidence-gate.md`. Validate with `validate-review-report.sh`. Coverage: `graphify:…`, `review_learnings:…`, when in scope `interaction_replay:…` (**R7**), `figma_markup:…` (**F7**), `narrow_viewport:…` (**V4**), and `null_safety_callers:…` (**N1**).
...
15. **Teach-review miss:** after the validated report is shown, ask via skill `hitl-choice` preset **Teach-review miss** (`miss` / `project_secret` / `no_miss`). Recommended: `miss`. **Never edit kit git** in this orchestrator. Do not auto-capture. Do not read/write ledgers in the orchestrator.
...
- `review-learn` (`mode:load` before phases; `mode:capture` only after `project_secret`)
...
- Never load full third-party skill text, ledger markdown, or R1–R7 / F1–F7 / V1–V4 checklist bodies into this orchestrator context — phases and `review-learn` own those reads.
