...
assert_eq pass_test_count "4" "$(json_field "$pass_json" 'd["test_count"]')"
...
assert_contains pass_has_review_quality "$(json_field "$pass_json" '" ".join(t["name"] for t in d["tests"])')" "review-response-quality-fixtures"
...
assert_eq pass_review_quality "True" "$(json_field "$pass_json" 'd.get("metrics", {}).get("review_response_quality", {}).get("ok") is True')"
...
assert_contains pass_out_review_quality "$(cat "$TMP/pass.out")" "Metrics review_response_quality:"
