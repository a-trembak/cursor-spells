...
9. If **Needs clarification** is non-empty, stop and ask via skill **`hitl-choice`** preset **Engineer-review clarify** (sequential `AskQuestion` per `C#`; recommended option labeled; tokens `C1:A`; batch text like `C1: A; C2: B` is a **reply shape only**). Each sequential question repeats that item’s Context, What, When it shows up, File, Lines, Jump, and numbered fence. On answers, re-dispatch affected phases and re-emit with the same evidence bar.
