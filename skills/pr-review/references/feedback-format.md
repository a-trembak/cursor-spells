...
If **Needs clarification** is non-empty, after the validated report ask via skill **`hitl-choice`** preset **Engineer-review clarify** (sequential `AskQuestion` per `C#`; recommended option labeled). Each sequential `C#` question repeats that item’s **Context**, **What**, **When it shows up**, **Where** block and numbered code fence. Batch letter lists are a **reply shape only**. Text fallback:
