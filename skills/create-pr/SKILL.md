...
9. **Trajectory score** (after Pipeline finale was asked, before applying `ready` / `ready_jira`):
   ...
   - On `FAIL`: print the `FAIL` lines and stop. At that stop, ask skill `hitl-choice` preset **Trajectory fail**. On `skip`, mention `/capture-escape` with the `FAIL` lines and do not start `engineer-reviewer`. On `generalize`, follow `evals/trajectories/README.md` “Add a case” only when the current git root contains both `skills/engineer-review` and `agents/engineer-reviewer.md`; otherwise print the `FAIL` log and stop. After validate, show the case JSON or diff and wait for the human to confirm it is correct before `git commit`. Do not run `gh pr ready`. Do not merge.
