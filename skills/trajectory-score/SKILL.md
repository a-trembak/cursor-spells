...
description: >-
  ...
  natural stop. Use from /start-task, /start-issue-task, create-pr, finish-plan,
...
Kit-owned **harness loop**: golden case → session or slice ledger → hard-sensor `score` at the moment the contract is provable → on `FAIL`, ask **Trajectory fail**. Evaluation measures the **agent path**, not product-code taste. `engineer-reviewer` stays the diff reviewer.
...
Print the `FAIL` lines and **stop**. Ask skill `hitl-choice` preset **Trajectory fail**. On `skip`, mention `/capture-escape` with the `FAIL` lines and do not start `engineer-reviewer`. On `generalize`, follow `evals/trajectories/README.md` “Add a case” only when the current git root contains both `skills/engineer-review` and `agents/engineer-reviewer.md`; otherwise print the `FAIL` log and stop. After validate, display the validated case JSON (the file contents) in chat and wait for the human to confirm it is correct before `git commit`. Do not offer `git diff` as a substitute. Do not invent `approve-spec` / `skip` / `ready`. Do not run `gh pr ready`. Do not merge.
...
1. **Init once** at pipeline start with the matching end-to-end case input (`full-happy-path`, `fast-skips-plan-layer`, `issue-happy-path`). Use the case's exact invocation (`/start-task PROJ-1`, `/start-task --fast PROJ-1`, `/start-issue-task PROJ-1`) — not the live ticket key.
...
4. Init `session-issue` only when `jira_class` is `bug` (matches `issue-happy-path`). Explicit `/start-issue-task` on a Story/Task (`jira_class` feature) scores the slice `start-issue-story-stays-issue` instead.
...
|------|------|------------|
| `fetch-failure-stops` | Jira fetch failed; stop for pasted text | `--invocation "/start-task PROJ-1" --fetch fail`. Skip if caller was `/start-issue-task` or `/write-tech-spec` |
| `route-unknown-asks-human` | After Pipeline route was asked (`jira_class` unknown, no `--fast`) | `--invocation "/start-task PROJ-1" --fetch ok --jira-class unknown`. Gate tokens `full,fast,issue` |
| `route-feature-to-full` | After tech-spec-entry was asked (feature, no `--fast`) | `--jira-class feature`. Gate tokens `human,agent` |
| `route-bug-to-issue` | After issue-fix-plan starts from `/start-task` without re-fetch | `--jira-class bug`. Do not ask pipeline-route / fast-vs-issue / tech-spec-entry |
| `fast-bug-asks-human` | After Fast vs issue was asked (`--fast` + bug) | `--invocation "/start-task --fast PROJ-1" --jira-class bug`. Tokens `issue,stay_fast` |
...
- `evals/` is kit-only. Dogfood does not require live `/start-task` inside kit continuous integration.
