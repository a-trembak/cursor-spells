...
1. **Discover** candidate repos (see Discovery precedence below).
2. **Detect changed repos** (see Changed-repo detection) for each discovered path.
3. **Route:**
   - If changed repo count **< 2** → invoke `engineer-reviewer` for the current repo only and **stop**. Single-repo behavior is unchanged; the supervisor never engages.
...
`finish-plan` and manual `/multi-review` both use this algorithm before any review phases run.

## Discovery precedence

Resolve the repo set in this order; stop at the first source that succeeds:

1. **Graphify (preferred).** Workspace-level build under the parent folder that contains sibling repos (`graphify-out/`). Query for repo map and cross-repo impact. **Do not create `multi-repo.json`** when graphify answers successfully.
2. **Parent `.cursor/multi-repo.json` (fallback).** Read `<workspace-parent>/.cursor/multi-repo.json` if graphify is absent or unqueryable. Regenerate only when a listed path is missing or the caller passes `--refresh`.
3. **Explicit paths (override, v1).** `/multi-review <path...>` supplies repos directly for that run; stack heuristics still apply per path.
...
If both `test` checks fail, fall through to `multi-repo.json` or the fallback scan.

## Fallback scan

When graphify is not installed or not built:
...
3. Write **`<workspace-parent>/.cursor/multi-repo.json`** (never inside a single leaf repo):
...
Paths are relative to the workspace parent. Later runs read this file; regenerate only if a listed path is missing or `--refresh` is requested.
...
A repo is **changed** if either command reports any output. Collect all changed repos; apply the routing rule (≥ 2 → supervisor).
