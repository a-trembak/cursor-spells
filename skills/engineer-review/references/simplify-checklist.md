...
| **Presentation-only unit tests** | Diff adds or keeps tests that only assert fill/palette hex, border color/style, fonts, opacity, MUI `sx`, `toHaveStyle`, chart chrome, Emotion snapshots, or cosmetic stylesheet XML with no domain decision (Java **or** React) | `P1` clarify — drop; see [business-logic-tests-checklist.md](business-logic-tests-checklist.md) **T1** |
