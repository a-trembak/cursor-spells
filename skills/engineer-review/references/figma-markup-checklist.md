...
When the figma phase is in scope (frontend, not `user_said_no_figma`), engineer-review Coverage **must** include:
...
- `skipped` — frontend, but the checklist was not walked: URLs still missing (`awaiting_figma_urls`), or URLs were given and the walk did not run (treat as incomplete review)
