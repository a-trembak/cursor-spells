...
| security | Kit [security-hardening-checklist.md](security-hardening-checklist.md) (**S1–S10**) — only if diff touches auth, sessions, crypto, PII, SQL/NoSQL, network, file upload, secrets, SSRF/XSS sinks, deserialization; optional enrichment `security-review` if installed (never skip the kit checklist when missing) |
