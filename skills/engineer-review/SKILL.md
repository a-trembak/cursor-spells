...
13. Needs clarification → first demote any rule-answered items to Fixed per [`references/auto-fix-eligibility.md`](references/auto-fix-eligibility.md); remaining only → HITL **Engineer-review clarify**; each sequential question repeats File, Lines, Jump, and numbered fence; re-dispatch affected phases. R1 timing/host answers also re-dispatch logic + architecture with `interaction_replay` (phases load checklists — orchestrator does not).
...
- **Rule already answers:** if a loaded kit/project checklist, skill, or rule mandates the fix → Fixed / auto-apply (eligibility Yes) — **never** Needs clarification HITL to reconfirm that policy (comments domain-only and every other rule-mandated class).
