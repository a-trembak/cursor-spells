...
5. Route:
   ...
   - If changed repo count is **1**, run `engineer-reviewer` for that repo. Single-repo behavior is unchanged.
   - If changed repo count is **>= 2**, persist parent `.cursor/multi-repo.json` only when no explicit paths were supplied, graphify is absent or unqueryable, and the repo set came from a sibling scan (or `--refresh` was supplied). Then invoke agent `multi-repo-supervisor` with `explicit_paths` and `refresh` when present.
