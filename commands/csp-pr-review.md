...
5. Run shared `evidence-gate.md` (backfill or drop). Draft full **Findings** (**Context** + File/Lines/Jump/GitHub + numbered fence; Clarify items also **Options** + **Recommendation**) — **never** Verdict/Блокери digest. Validate with `$KIT/scripts/validate-review-report.sh`; rebuild until exit 0. Then emit; `english-humanizer` then `plain-language-chat` on prose. Point at the canvas if built. Optional PR comment draft only as appendix.
