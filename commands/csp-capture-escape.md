...
4. `project_secret` → read `skills/engineer-review/references/review-learn-protocol.md`. Invoke agent **`csp-review-learn`** with:
   - `mode: capture`
...
- Do not start `csp-engineer-reviewer`, `csp-bug-fixer`, or `/csp-start-issue-task` unless the human asks. If they also want a fix, they run `/csp-start-issue-task` or `/csp-start-task` separately.
