...
**Harness health / bench:** before changing skills or agents, run `bash scripts/harness-bench.sh` (all `scripts/tests/*.sh` plus trajectory validate + fixture score; writes `evals/harness/reports/<timestamp>.json` with `metrics.quality` pass rates and `metrics.speed` p50/p95/slowest). Inventory without a full bench: `python3 scripts/harness-health.py` or `/csp-harness-status` (orientation only; prints quality/speed when a bench report exists). See [`evals/harness/README.md`](evals/harness/README.md) and [`docs/superpowers/dogfood/harness-health-checklist.md`](docs/superpowers/dogfood/harness-health-checklist.md).
