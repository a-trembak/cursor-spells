...
| [`finish-plan`](skills/finish-plan/) | Plan→HITL handoff: `review-surface` (draft URL + `SetActiveBranch`) then review-gate; engineer-review still runs after |
...
1. When coding from a plan is done: `/finish-plan` (this is the **review-gate** HITL, not another planning step). First it applies [`review-surface`](skills/finish-plan/references/review-surface.md): check out the feature branch in each open folder, call `SetActiveBranch`, and open or reuse a **draft** GitHub pull request so you get a URL to open in Cursor. That draft is **not** the pipeline end (no Pipeline finale yet).
...
6. `review-gate` via `/finish-plan` — draft URL + `SetActiveBranch`, then **HITL** `skip`/`approve`/`done` (or `fixes` back to `software-developer`). Pipeline continues.
