...
- `/csp-start-task` never invents an answer at any HITL gate (tech-spec entry question, Blocker/Decision questions, `approve-spec`/`revise`/`skip`, blocked critique, `finish-plan`'s `skip`/`approve`/`done`, review clarifications) — it only removes the need to manually re-invoke each stage's command.
...
- Do not modify `skills/tech-spec/`, `agents/csp-tech-spec.md`, `skills/implementation-critic/`, `agents/csp-implementation-critic.md`, `skills/finish-plan/`, or `commands/csp-finish-plan.md` — all already shipped and reviewed; `/csp-start-task` and `start-build` only *invoke* them, they don't change their contracts.
...
- `commands/csp-start-build.md` (new) — `/csp-start-build [plan-path]` manual entry point
...
- `commands/csp-start-task.md` (modified, full replacement) — becomes the pipeline orchestrator
...
2. **Auto-run the critique** — no HITL needed to start it, `implementation-critic` is read-only:
   - Invoke skill `implementation-critic` (or `/csp-critique-plan <plan-path>`) against the plan.
3. **On `Verdict: clear`:**
...
- Manual `/csp-critique-plan` still works standalone for an ad-hoc look; this skill is the enforced pre-build path.
```

- [ ] **Step 2: Verify structure**

Run: `grep -c "^name: start-build$" skills/start-build/SKILL.md && grep -c "build-gate.pending" skills/start-build/SKILL.md && grep -c "csp-implementation-critic" skills/start-build/SKILL.md && grep -c "Verdict: clear" skills/start-build/SKILL.md && grep -c "subagent-driven-development" skills/start-build/SKILL.md`
Expected: `1`, `2`, `4`, `1`, `3`
...
### Task 2: Create the `/csp-start-build` command

**Files:**
- Create: `commands/csp-start-build.md`
...
Run: `grep -c "start-build" commands/csp-start-build.md && grep -c "build-gate.pending" commands/csp-start-build.md && grep -c "csp-implementation-critic" commands/csp-start-build.md`
Expected: `2`, `1`, `2`

- [ ] **Step 3: Commit**

```bash
git add commands/csp-start-build.md
git commit -m "Add /csp-start-build command"
```
...
2. If you cannot load that skill, still:
   ...
   - Run `implementation-critic` / `/csp-critique-plan` against the plan
...
Does not apply to pure Q&A, docs-only answers, or when the user already launched `/csp-start-build` or `/csp-critique-plan` manually and is now explicitly asking to proceed.
```
...
### Task 6: Rewrite `/csp-start-task` as the pipeline orchestrator

**Files:**
- Modify: `commands/csp-start-task.md` (full replacement)

**Interfaces:**
- Consumes: `tech-spec` (already shipped), `writing-plans` (external superpowers skill), `start-build` (Task 1), `subagent-driven-development` (external superpowers skill), `finish-plan` (already shipped), `csp-engineer-reviewer` (already shipped).
...
2. **Tech spec** — invoke skill `tech-spec` (agent `csp-tech-spec`) with the AC source, the patterns file path (if found), and the detected stack label:
...
Run: `grep -c "^## Pipeline" commands/csp-start-task.md && grep -c "^[0-9]\. \*\*" commands/csp-start-task.md && grep -c "HITL" commands/csp-start-task.md && grep -c "start-build" commands/csp-start-task.md`
Expected: `1`, `7`, `11`, `1`
...
git commit -m "Rewrite /csp-start-task as the full pipeline orchestrator"
```
...
install_user_bits() {
  ...
  link_or_copy "$KIT_ROOT/commands/csp-engineer-review.md" "$HOME/.cursor/commands/csp-engineer-review.md"
  link_or_copy "$KIT_ROOT/commands/csp-finish-plan.md" "$HOME/.cursor/commands/csp-finish-plan.md"
  link_or_copy "$KIT_ROOT/commands/csp-multi-review.md" "$HOME/.cursor/commands/csp-multi-review.md"
  link_or_copy "$KIT_ROOT/commands/csp-start-task.md" "$HOME/.cursor/commands/csp-start-task.md"
  link_or_copy "$KIT_ROOT/commands/csp-write-tech-spec.md" "$HOME/.cursor/commands/csp-write-tech-spec.md"
  link_or_copy "$KIT_ROOT/commands/csp-critique-plan.md" "$HOME/.cursor/commands/csp-critique-plan.md"
  link_or_copy "$KIT_ROOT/commands/csp-start-build.md" "$HOME/.cursor/commands/csp-start-build.md"
  link_or_copy "$KIT_ROOT/agents/csp-engineer-reviewer.md" "$HOME/.cursor/agents/csp-engineer-reviewer.md"
  link_or_copy "$KIT_ROOT/agents/csp-multi-repo-supervisor.md" "$HOME/.cursor/agents/csp-multi-repo-supervisor.md"
  link_or_copy "$KIT_ROOT/agents/csp-tech-spec.md" "$HOME/.cursor/agents/csp-tech-spec.md"
  link_or_copy "$KIT_ROOT/agents/csp-implementation-critic.md" "$HOME/.cursor/agents/csp-implementation-critic.md"
  local f
...
if [[ -n "$PROJECT" ]]; then
  ...
  echo "next: open the project in Cursor → /csp-start-task to run the whole pipeline, /csp-finish-plan after plans, /csp-start-build before executing a plan manually, /csp-engineer-review anytime"
fi
...
```markdown
Slash commands after install: /csp-finish-plan  /csp-engineer-review
```

to:

```markdown
Slash commands after install: /csp-start-task  /csp-finish-plan  /csp-start-build  /csp-engineer-review
```
...
**Interfaces:**
- Consumes: skill path `skills/start-build/`, commands `/csp-start-task` and `/csp-start-build` from Tasks 1, 2, 6.
...
`/csp-start-task [ac-source]` orchestrates the whole pipeline end-to-end, stopping only at the human-in-the-loop (HITL) gates that already exist — it never skips or softens any of them:
...
4. Runs the pre-build critique gate (`/csp-start-build`, below) — automatic start, **HITL** only if findings block
5. Executes the plan via `subagent-driven-development` — automatic, no "which approach?" prompt in this flow
6. `/csp-finish-plan` — **HITL** `skip`/`approve`/`done`
...
`/csp-start-build [path]` (or the `start-build` skill, auto-invoked by `/csp-start-task`) always runs `implementation-critic` before Task 1 of a plan is dispatched — no permission needed to start the critique itself, since it's read-only. If `Verdict` comes back `blocked` or `clear pending accept`, it stops and waits for a plan revision or `accept F<id>` replies before execution begins.
```
...
**1. Spec coverage:** Every point confirmed with the user is covered: `start-build` auto-runs the critique without asking permission to start (only gates on `Verdict`), mirrors `finish-plan`'s skill+rule+hook shape exactly (Tasks 1, 3, 4, 5), `/csp-start-task` chains all seven pipeline stages with HITL preserved at exactly the pre-existing gates and nowhere else (Task 6), and the previously-undiscovered gap — `install-to-project.sh` never wiring `tech-spec`/`implementation-critic`/`code-comments` into a consumer install — is fixed (Task 7), since none of the orchestration in Task 6 would actually be installable without it.
...
**3. Type/name consistency:** `start-build` and `.cursor/build-gate.pending` are spelled identically across Tasks 1–7. The already-shipped `tech-spec`, `implementation-critic`, `code-comments`, `finish-plan`, and `csp-engineer-reviewer` names are referenced only by their real, existing paths (verified against the already-committed files, none of which this plan modifies) — no paraphrased variants. All `grep` verification counts in this plan were computed against real scratch-file drafts before being written down.
