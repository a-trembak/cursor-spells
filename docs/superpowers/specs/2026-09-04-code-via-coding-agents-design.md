...
Pipeline commands (`/csp-start-task`, `/csp-start-build`, `/csp-start-issue-task`) already dispatch `csp-software-developer` or `csp-bug-fixer`. Ad-hoc “write / fix this in chat” requests still tempt the parent agent to edit product trees directly, skipping branch setup, skill-map routing, verification, and (for bugs) the debug evidence gate.
...
| Ticket bugs / `/csp-start-issue-task` | Nested Task **`csp-bug-fixer`** / skill `bug-fix` |
...
| Pipeline rewrites | **Out of scope** — `/csp-start-task` and `/csp-start-issue-task` already dispatch correctly; this rule closes the ad-hoc gap |

## Routing (normative)

1. If the human asks to implement a feature, apply an accepted plan task, apply review-gate `fixes`, or run `/csp-start-task --fast` → dispatch **`software-developer`** and wait for it to return.
2. If the human asks to fix a bug from an issue path or `/csp-start-issue-task` → dispatch **`csp-bug-fixer`** and wait for it to return.
...
- Merging `csp-bug-fixer` into `software-developer`
...
- A parent agent following kit rules does not open a product implementation edit without first dispatching `software-developer` or `csp-bug-fixer` as appropriate.
